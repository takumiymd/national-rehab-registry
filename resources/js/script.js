...
function runQuery(event, resultId) {
  ...
  resultBox.innerHTML = '<p style="color: #666; font-size: 13px;">Running...</p>';
  ...
  xhr.onload = function() {
    if (xhr.status === 200) {
      resultBox.innerHTML = '<p class="result-label">Result</p>' + xhr.responseText;
    } else {
      resultBox.innerHTML = '<p class="result-msg error">Server error: ' + xhr.status + '</p>';
    }
  };

  xhr.onerror = function() {
    resultBox.innerHTML = '<p class="result-msg error">Could not connect. Make sure XAMPP is running.</p>';
  };
...
function updateSQL1() {
  ...
  document.getElementById('sql1').textContent = 'SELECT ' + field + '\nFROM ' + table;
}
...
function updateSQL2() {
  ...
  document.getElementById('sql2').textContent = 'SELECT ' + field + '\nFROM ' + table + '\nWHERE ' + cField + " = '" + cVal + "'";
}
...
function updateSQL3() {
  ...
  document.getElementById('sql3').textContent = 'SELECT *\nFROM ' + t1 + '\nJOIN ' + t2 + '\nON ' + t1 + '.' + f1 + ' = ' + t2 + '.' + f2;
}
...
function updateSQL5a() {
  ...
  document.getElementById('sql5a').textContent = 'SELECT ' + fn + '(' + field + ') AS CountResult\nFROM ' + table;
}
...
function updateSQL5b() {
  ...
  document.getElementById('sql5b').textContent = 'SELECT ' + fn + '(' + field + ') AS MaxResult\nFROM ' + table;
}
...
function updateSQL6() {
  ...
  document.getElementById('sql6').textContent = 'SELECT ' + groupField + ', ' + fn + '(' + aggField + ') AS AggregatedValue\nFROM ' + table + '\nGROUP BY ' + groupField;
}
...
function updateSQL7() {
  ...
  document.getElementById('sql7').textContent = 'DELETE FROM ' + table + "\nWHERE " + field + " = '" + val + "'";
}
...
function updateSQL8() {
  ...
  document.getElementById('sql8').textContent = 'UPDATE ' + table + "\nSET " + updateField + " = '" + newVal + "'\nWHERE " + idField + " = '" + idVal + "'";
}
